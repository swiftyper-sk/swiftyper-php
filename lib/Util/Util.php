...
abstract class Util
{
    ...
    /**
     * Converts a response from the Swiftyper API to the corresponding PHP object.
     *
     * @param array $resp the response from the Swiftyper API
     * @param array $opts
     *
     * @return Collection|SwiftyperObject
     */
    public static function convertToSwiftyperObject($resp, $opts)
    {
       ...
       if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
                $mapped[] = self::convertToSwiftyperObject($i, $opts);
            }

            return Collection::constructFrom($mapped);
        }
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                $class = $types[$resp['object']];
            } else {
                $class = \Swiftyper\SwiftyperObject::class;
            }
        ...
        if (null === self::$isMbstringAvailable) {
            ...
            if (!self::$isMbstringAvailable) {
                \trigger_error('It looks like the mbstring extension is not enabled. ' .
                    ...
                    'podpora@swiftyper.sk if you have any questions.', \E_USER_WARNING);
            }
        }
    ...
    {
        if ($h instanceof \Swiftyper\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
