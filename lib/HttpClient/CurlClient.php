...
namespace Swiftyper\HttpClient;

use Swiftyper\Exception;
...
class CurlClient implements ClientInterface
{
    ...
    /** @var \Swiftyper\Util\RandomGenerator */
     ...
     * @param null|\Swiftyper\Util\RandomGenerator $randomGenerator
     */
     ...
     *   <li>\Swiftyper\Util\CaseInsensitiveArray $rheaders The response headers</li>
    ...
    public function request($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        // Swiftyper's API servers are only accessible over IPv4. Force IPv4 resolving to avoid
        // potential issues (cf. https://github.com/swiftyper/swiftyper-php/issues/1045).
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case \CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Swiftyper ({$url}).  Please check your "
                 . 'internet connection and try again.  If this problem persists, '
                 . "you should check Swiftyper's service status at "
                 . 'https://swiftyper.statuspage.io, or';

                break;

            default:
                $msg = 'Unexpected error communicating with Swiftyper.  '
                 . 'If this problem persists,';
        }
        $msg .= ' let us know at podpora@swiftyper.sk.';
     ...
     * @param int $errno
     * @param int $rcode
     * @param array|\Swiftyper\Util\CaseInsensitiveArray $rheaders
     * @param int $numRetries
     *
     * @return bool
     */
    ...
    {
        if ($numRetries >= Swiftyper::getMaxNetworkRetries()) {
            return false;
        }
        ...
        if (isset($rheaders['swiftyper-should-retry'])) {
            if ('false' === $rheaders['swiftyper-should-retry']) {
                return false;
            }
            if ('true' === $rheaders['swiftyper-should-retry']) {
                return true;
            }
        }
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = \min(
            Swiftyper::getInitialNetworkRetryDelay() * 1.0 * 2 ** ($numRetries - 1),
            Swiftyper::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = \max(Swiftyper::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? (float) ($rheaders['retry-after']) : 0.0;
        if (\floor($retryAfter) === $retryAfter && $retryAfter <= Swiftyper::getMaxRetryAfter()) {
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which Swiftyper use.
